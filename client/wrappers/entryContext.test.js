...
test("puts own props on child props", () => {
  ...
  const EntryContextWrappedComponent = makeEntryContextWrapper()(TestComponent);
...
test("puts entry context value on child props", () => {
  const contextProvider = ({ children }) => {
    return (
...
test("calls setEntry on retrieveEntryById success", () => {
...
test("logs error on retrieveEntryById rejection", () => {
...
test("calls setEntry and redirects on addEntry success", () => {
  const formData = {
    name: "mocked entry 1",
    link: "https://mocked.link.com/1",
    description: "mocked description 1",
...
test("logs error on addEntry rejection", () => {
  const formData = {
...
test("calls setEntry on selectEntry", () => {
  const mockSetEntryContext = jest.fn();
  const contextProvider = ({ children }) => {
    return (
      <EntryContext.Provider value={[{}, mockSetEntryContext]}>
